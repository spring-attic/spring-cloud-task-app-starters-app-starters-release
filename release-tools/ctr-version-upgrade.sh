...
for repo in "${ARRAY[@]}" ; do
 ...
 # BOM parent update
 ./mvnw versions:update-parent "-DparentVersion=[0.0.1,$3]" -Pspring -DgenerateBackupPoms=false -pl :composedtaskrunner-task-app-dependencies

 ./mvnw versions:set -DnewVersion=$1 -DgenerateBackupPoms=false
 ./mvnw versions:set -DnewVersion=$1 -DgenerateBackupPoms=false -pl :composedtaskrunner-task-app-dependencies
 ./mvnw versions:update-parent -DparentVersion=$2 -Pspring -DgenerateBackupPoms=false -pl \!composedtaskrunner-task-app-dependencies
